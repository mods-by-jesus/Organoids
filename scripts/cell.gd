...
func _apply_genes():
	...
	$CollisionShape2D.scale = Vector2.ONE * vis_scale
	
	if is_instance_valid(body_visual):
		body_visual.scale = Vector2.ONE * vis_scale
		body_visual.position = -body_visual.size / 2.0 * vis_scale # Центрируем визуальное тело
	...
	if body_visual and body_visual.material:
		body_visual.material.set_shader_parameter("base_color", Color(r, g, b, 1.0))
		...
		body_visual.material.set_shader_parameter("visual_padding", 1.65)
...
func _find_nearest_target():
	...
	if can_eat_floor_food():
		var areas = wm.get_food_near(global_position, _get_effective_vision()) if wm and wm.has_method("get_food_near") else vision_area.get_overlapping_areas()
		for a in areas:
	...
	if _can_hunt_cells():
		var bodies = wm.get_cells_near(global_position, _get_effective_vision()) if wm and wm.has_method("get_cells_near") else vision_area.get_overlapping_bodies()
		for b in bodies:
...
func split():
	...
	for key in new_genes:
		if randf() < genes.mutation_rate * 3.0:
			...
			elif key in ["membrane_roughness", "membrane_asymmetry", "nucleus_size", "bioluminescence", "phagocytosis", "shape_elongation", "shape_spikiness", "shape_amoeboid"]:
...
func _create_split_bud(new_genes: Dictionary):
	...
	if split_bud.material:
		...
		var r = lerp(0.1, 1.0, new_genes.aggressiveness)
		var g = lerp(0.8, 0.1, new_genes.aggressiveness)
		var b = lerp(0.2, 0.75, new_genes.phagocytosis)
		var effective_new_speed = new_genes.speed / pow(clamp(new_genes.size, MIN_SIZE, MAX_SIZE), 0.85)
		split_bud.material.set_shader_parameter("base_color", Color(r, g, b, 1.0))
		...
		split_bud.material.set_shader_parameter("visual_padding", 1.65)
...
func _finish_split(new_genes: Dictionary):
	...
	offspring.scale = Vector2.ONE * _get_visual_size_scale(new_genes.size)
	_detach_split_bud()
